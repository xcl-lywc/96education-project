<template>
	<div>
		<div class="custom-avatar" @click="isDisabled?'':cropDialogVisible_=true;resetAvatar?option.img=resetAvatar:option.img='';clear()">
			<div class="el-upload-avatar">
				<img v-if="resetAvatar" :src="resetAvatar" class="avatar" :class="size"/>
...
export default {
  ...
  methods: {
    ...
    submitUpload(file) {
      ...
      this.finish('Blob');// 二进制
      // this.finish('Data');// base64
    },
      ...
      if (type === "Blob") {
        this.$refs.cropper.getCropBlob(data => {
          ...
          var formData = new FormData();
	        ...
	        this.axios.post(`${this.common.basePath}/file/public/upload`,formData).then((response) => {
	        	let result = {}
		        if(response.data.data) {
		          result = {
		            name: response.data.data[0],
		            path: response.data.data[0],
		          }
		        }
